...
//       .map((image) => `<img class="plant__image" src="${image}" alt="" />`)
//       .join('')}
//     ${recommendation.extraImages
//       .map((image) => `<img class="plant__image" src="${image}" alt="" />`)
//       .join('')}
...
function renderRecommendation(recommendation) {
  ...
  // Verificar si la recomendación es válida y tiene extrasImages
  ...
  recommendationDiv.innerHTML = `
    ...
    <img class="plant__image image__position" src="${recommendation.potImage}" alt="" />
    ${recommendation.extrasImages
      .map((image) => `<img class="extra__image" src="${image}" alt="" />`)
      .join('')}
    <ul class="plant__details">
      <li>Name: ${recommendation.name}</li>
      <li>Soil: ${recommendation.soil}</li>
